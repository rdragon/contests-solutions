...
let o', a = List.init (m*n) (fun i -> let x, y = i%m, i/m in i=1 || i=m*n-2 || not (x=0 || y=0 || x=m-1 || y=n-1)), Set.singleton
...
let p _ = is <- Seq.collect (fun i -> [i;i-1;i+1;i-m;i+m] |> List.filter (fun j -> j > 0 && j < o.Length && o[j])) is |> Set.ofSeq
let rec q i k = if is.Contains(i) then is <- a i; k else o <- os[(k+1) % os.Length]; p(); q i (k+1) in q (m*n-2) 0 |> q 1 |> q (m*n-2)
